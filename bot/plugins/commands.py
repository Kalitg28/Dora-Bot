...
@Client.on_message(filters.command(["json2","json2@DoraFilterBot"]) & filters.private, group=4)
async def user_json(bot:Bot, update:Message):

    if update.reply_to_message:
        msg = await bot.USER.get_messages(update.reply_to_message.forward_from_chat.id, update.reply_to_message.forward_from_message_id)
        string = str(msg)
    else :
        string = str(update)
